...
If nothing new is found, return []."""
    ...
    print(f"DEBUG: {len(text_blocks)} text block(s) returned from Claude")
    if not text_blocks:
        print("DEBUG: No text blocks found — Claude may have only returned tool calls")
        return []

    final_text = text_blocks[-1].strip()
    print(f"DEBUG: Last text block (first 500 chars):\n{final_text[:500]}")
    ...
    try:
        ...
        print(f"DEBUG: Parsed {len(postings)} postings from JSON")
    except json.JSONDecodeError as e:
        print(f"DEBUG: JSON parse failed ({e}), trying regex fallback")
        match = re.search(r"\[.*\]", final_text, flags=re.DOTALL)
        if not match:
            print("DEBUG: No JSON array found in response at all")
            return []
        try:
            postings = json.loads(match.group(0))
            print(f"DEBUG: Regex fallback parsed {len(postings)} postings")
...
def main():
    ...
    if force_test:
        print("FORCE_TEST mode: ignoring seen list, results won't be saved to seen file")
        seen = set()
    else:
